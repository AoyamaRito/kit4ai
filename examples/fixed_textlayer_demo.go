...
func main() {
	...
	textLayer.WriteTextWithWidth(2, 5, "Hello World")  // 半角スペースを含む
	
	// テスト2: 中央寄せ（幅計算は維持）
	textLayer.WriteTextCentered(2, 7, 36, "中央寄せテスト")
	textLayer.WriteTextCentered(2, 8, 36, "Center Test")
	
	// テスト3: 右寄せ
	textLayer.WriteTextRight(2, 10, 36, "右寄せテスト")
	textLayer.WriteTextRight(2, 11, 36, "Right Test")
	
	// テスト4: 幅計算確認
	textLayer.WriteTextWithWidth(2, 13, "幅計算確認用")
	textLayer.WriteTextWithWidth(2, 14, "Width Check")
	...
	fmt.Println("\n修正内容:")
	fmt.Println("- WriteTextWithWidth: 連続配置（全角間の空白なし）")
	fmt.Println("- 半角スペースは正常に配置される")
